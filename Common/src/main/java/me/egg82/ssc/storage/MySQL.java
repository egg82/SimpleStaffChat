...
public class MySQL extends AbstractSQL {
    ...
    public static class Builder {
        ...
        public MySQL build() throws IOException, SQLException {
    ...
    public Set<ChatResult> getQueue() throws SQLException {
        ...
        SQLQueryResult result = sql.call("call `" + prefix + "get_queue_id`(?, ?);", lastMessageID, longServerID);
    ...
    public Set<ChatResult> getByPlayer(UUID playerID, int days) throws SQLException {
        ...
        Set<ChatResult> retVal = new LinkedHashSet<>();
        SQLQueryResult result = sql.call("call `" + prefix + "get_messages_player`(?, ?);", longPlayerID, days);
        for (Object[] row : result.getData()) {
    ...
    public void setLevelRaw(byte level, String name) throws SQLException { sql.execute("INSERT INTO `" + prefix + "levels` (`id`, `name`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `name`=?;", level, name, name); }

    public void setServerRaw(long longServerID, UUID serverID, String name) throws SQLException { sql.execute("INSERT INTO `" + prefix + "servers` (`id`, `uuid`, `name`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `uuid`=?, `name`=?;", longServerID, serverID.toString(), name, serverID.toString(), name); }
    ...
    public void postRaw(long postID, long longServerID, long longPlayerID, byte level, String message, long date) throws SQLException { sql.execute("INSERT IGNORE INTO `" + prefix + "posted_chat` (`id`, `server_id`, `player_id`, `level`, `message`, `date`) VALUES (?, ?, ?, ?, ?, ?);", postID, longServerID, longPlayerID, level, message, date); }

    public void setLevel(byte level, String name) throws SQLException { sql.execute("INSERT INTO `" + prefix + "levels` (`level`, `name`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `name`=?;", serverID, name, name); }

    public void setServerName(String name) throws SQLException { sql.execute("INSERT INTO `" + prefix + "servers` (`uuid`, `name`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `name`=?;", serverID, name, name); }
